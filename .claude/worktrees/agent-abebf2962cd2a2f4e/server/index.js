...
async function classifyIntent(message) {
  if (!GEMINI_API_KEY) {
    return { intent: 'general_health_advice', params: {}, confidence: 0.5 };
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn('[Aurora] Gemini intent classification HTTP error:', res.status, errText.slice(0, 200));
    ...
    console.warn('[Aurora] Intent classification failed:', err?.message);
  }
...
async function runAgentPipeline(message, userId, context, chatHistory) {
  ...
  // ── Step 4: Generate reply (Gemini → OpenAI → deterministic)
  let reply = null;
...
app.listen(port, '0.0.0.0', () => {
  ...
  console.log(`   AI:            ${GEMINI_API_KEY ? 'Gemini 2.0 Flash Lite'   : openai ? 'OpenAI fallback' : 'Deterministic fallback'}`);
